✅ Función de Validación - Propiedad

```php
/**
 * Valida los datos de una propiedad antes de ser guardada en la base de datos.
 * 
 * @param array $data Datos sanitizados de la propiedad
...
function validarPropiedad($data) {
    ...
    // CANTIDAD AMBIENTES
    if ($data['cantidad_ambientes'] < 1 || $data['cantidad_ambientes'] > 10) {
        $errores[] = "Debe ser un numero entre 1 y 10 (cantidad_ambientes)";
    }

    // CANTIDAD DORMITORIOS
    if ($data['cantidad_dormitorios'] < 1 || $data['cantidad_dormitorios'] > 10) {
    ...
    // CANTIDAD BAÑOS
    if ($data['cantidad_banos'] < 1 || $data['cantidad_banos'] > 10) {
